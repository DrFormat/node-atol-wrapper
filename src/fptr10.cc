...
NAN_MODULE_INIT(Fptr10::Init) {
  ...
  Nan::SetPrototypeMethod(ctor, "version", Version);
  Nan::SetPrototypeMethod(ctor, "beep", Beep);
...
NAN_METHOD(Fptr10::Version){
  const char *version_string = libfptr_get_version_string();
  info.GetReturnValue().Set(Nan::New(version_string).ToLocalChecked());
}

NAN_METHOD(Fptr10::Beep){
  Fptr10* self = Nan::ObjectWrap::Unwrap<Fptr10>(info.This());
  libfptr_beep(self->fptr);
  info.GetReturnValue().Set(Nan::Undefined());
}
